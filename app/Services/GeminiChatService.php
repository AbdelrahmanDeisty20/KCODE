...
class GeminiChatService
{
    ...
    public function __construct()
    {
        ...
        $this->model = config('gemini.model', env('GEMINI_MODEL', 'gemini-1.5-flash'));
        $this->temperature = (float) config('gemini.temperature', 0.7);
        $this->maxTokens = (int) config('gemini.max_tokens', 1000);
    }

    /**
     * Handle chat request with Gemini AI.
     */
    public function ask(string $prompt, array $history = [], ?string $locale = null): array
    {
        ...
        $systemPrompt = $locale === 'ar'
            ? config('gemini.system_prompt_ar')
            : config('gemini.system_prompt_en');
        ...
        foreach ($history as $msg) {
            $role = ($msg['role'] ?? 'user') === 'model' || ($msg['role'] ?? 'user') === 'assistant' ? 'model' : 'user';
    ...
    /**
     * Fallback response if Gemini API key is missing or request fails.
     */
